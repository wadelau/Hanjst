...
/*
 * add compatible with window.console
 */
...
(function(window){ //- anonymous Hanjst main func bgn
	...
	if(!window.Hanjst){
		...
		/*
		 * check browser tag whether it contains old MSIE or not ....
		 */
        ...
        if(myNav != null && myNav.indexOf("msie") > -1){
            ...
            window.alert(errMsg); 
			console.log(errMsg); 
			console.log("UA:"+myNav);
            return errMsg;
        }
    }

	//- constants, read from the object window.Hanjst
	var parseTag = window.Hanjst.ParseTag; 
	var unParseTag = '__NOT' + parseTag;
	var tplVarTag = window.Hanjst.TplVarTag; 
	var jsonDataId = window.Hanjst.JsonDataId; 
	var logTag = window.Hanjst.LogTag+" "; 
	var isDebug = window.Hanjst.IsDebug;
	
	var timeCostBgn = 0;
    if(isDebug){ 
		timeCostBgn = (new Date()).getTime();
	}
	var pageJsonElement = document.getElementById(jsonDataId);
	
	/*
	 * check parent node
	 * if not BODY, throw an error.
	 */
	if(pageJsonElement){
		var parentName = pageJsonElement.parentNode.nodeName;
		if(parentName.toLowerCase() != 'body'){
			console.log(logTag+'Error! parentNode:['
				+ parentName
				+'] should be "BODY". Please consider to put '
				+ logTag
				+' codes block under <body>. 201906040809.');
		} 
	}
	...
	if(pageJsonElement){
		...
		//- designate this a a test marker
		if(!tplData['copyright_year']){
			tplData['copyright_year'] = (new Date()).getFullYear();
		}
		...
		for(var $k in tplData){
			...
			if($k != null && $k != ''){
				...
				if(typeof window[$k] != 'undefined'){
						console.log(logTag+': Variables:['
							+ $k
							+'] conflict error. Please consider rename one of window.'
							+ $k
							+' or pageJsonData. 201904202305.');
				}
				else{
	...
	if(isDebug){
		console.log(logTag+"aft parse copyright_year:"+$copyright_year);
	}
	...
	var renderTemplate = function(window, document, tplHTML){
		...
		while(match = embeddedRe.exec(tplRaw)){
			...
			if(isDebug){
				console.log(logTag+"found embedded tpl sentence:["
					+ matchStr
		...
		for(var $prei in tplSegmentPre){
			...
			if(tplRawNew.indexOf(unParseTag) > -1){ // literal scripts
				...
				while(match = scriptRe.exec(tplRawNew)){
					...
					if(srcPos > 0){
						isIncludeScript = firstContent.indexOf(matchStr)>-1 ? false : true;
					}
					else{
						isIncludeScript = firstContent.indexOf(exprStr)>-1 ? false : true;
					}
					if(matchStr.indexOf(' async') > -1){
						isAsync = true;
					}
					else{
						isAsync = false;
					...
					if(isAsync || isIncludeScript){
						if(exprStr != null && exprStr != ''){
							if(exprStr.indexOf('document.write') > -1){
								/* should skip */
								if(isDebug){
									console.log(logTag+"found 'document.write' and skip...");
								}
							}
							else{
								if(isAsync){
									...
									tplSegment.push('var tmpTimer'
										+ ipos
										+'=window.setTimeout(function(){try{'
										+'Hanjst.appendScript(\''+exprStr+'\', \'\');'
										+'}catch(tmpErr){if('+isDebug+'){console.log("'
										+ logTag
							...
							if(isAsync){
								...
								tplSegment.push('var tmpTimer'
									+ ipos
									+'=window.setTimeout(function(){try{ Hanjst.appendScript(\'\', \''
									+ matchStr+'\');' 
									+'}catch(tmpErr){if('+isDebug+'){console.log("'
									+ logTag
						...
						if(isDebug){
							console.log(logTag+"found scripts:["
								+ matchStr
								+"] skiped for isIncludeScript:["+isIncludeScript+"] or isAsync:["
								+ isAsync+"]");
						}
					}
				...
				if(hasScript){
					...
					if(isDebug){
						console.log(logTag + "no scripts:"+tplRawNew);
					}
				...
				while(match = tplRe.exec(segStr)){
					...
					if(exprStr.indexOf(tplVarTag) == 0){
						...
						if(exprStr.match(/(\+|\-|\*|\/|=|~|!|\()/gm)){
							...
							if(hasLoopElse){
								loopElseStr += "\"+"+exprStr+"+\""; 
								// why only this? allow limited support for variables in xxxelse scope.
							}
							else{
								tpl2codeArr.push("\ttpl2js.push("+exprStr+");");
							}
						...
						if(exprStr.match(/(^( )?(if|for|while|switch|case|break))(.*)?/g)){
							...
							if(tmpmatch = blockBeginRe.exec(exprStr)){
								...
								if(tmpmatch[2].indexOf('each ') == 0){ //- foreach
									...
									//- skip first else sentence
									tpl2codeArr.push('\tblockLoopCount += 1;');
									exprStr = '';  
									hasLoopElse = true;
								}
		...
		tpl2code = "try{ " + tpl2code 
			+"\n}\ncatch(e1635){ var errMsg=JSON.stringify(e1635, Object.getOwnPropertyNames(e1635));"
			+" console.log(e1635);"
            + "var tmpRegexp=/>\:([0-9]+)\:([0-9]+)/gm; tmpRegexp2=/\"lineNumber\":([0-9]+)/gm; "
			+"var tmpmatch=null; var tmpLineno=0; var tmpCharno=0; "
			+"if(tmpmatch=tmpRegexp.exec(errMsg)){ tmpLineno=parseInt(tmpmatch[1]);"
			+" tmpCharno=tmpmatch[2];}else if(tmpmatch=tmpRegexp2.exec(errMsg)){"
			+"tmpLineno = parseInt(tmpmatch[1]);}; if(tmpLineno>0){"
			+"var tmpArr=Hanjst.tpl2code.split(\"\\n\"); "
			+"errMsg += \"<p>Line \"+(tmpLineno-1)+\": \"+tmpArr[tmpLineno-4].replace(/</g,"
			+" '&lt;')+\"</p>\"; errMsg += \"<p>Line \"+tmpLineno+\": "
			+"\"+tmpArr[tmpLineno-3].replace(/</g, '&lt;')+\"</p>\"; "
			+"errMsg += \"<p>Line \"+(tmpLineno+1)+\": \"+tmpArr[tmpLineno-2].replace(/</g,"
			+" '&lt;')+\"</p>\"; console.log('errMsg:['+errMsg+']'); }else{"
			+" errMsg+=\"regExp:\"+tmpRegexp+\" failed.\"; }\n"
        ...
        if(true){
            ...
            if(loadingLayer){
				...
				if(isDebug){
					console.log((new Date())+" "+logTag+" loadingLayer is quiting....");
				}
            }
	...
	var appendScript = function(myCode, myElement) {
		...
		if(isDebug){
			console.log(logTag+(new Date())+' appendScript: ['
				+myCode+']/['+myElement+'] has been appended.');
		}
	};
	...
	var _parseTagInElement = function(exprStr, match){
		...
		if((endIfPos > 0 && startIfPos > 0
			...
			if(isDebug){
				console.log("exprStr:["+exprStr+"] tmpi:"+tmpi+" aft sorted:["
					+parts2.join(' ')+"]");
			}
			exprStr = parts2.join(' '); // left parts3
			var fields = _searchField(parts2);
			var newExprStr = " {if "+fields['condition']+" "+fields['op']+" "
				+fields['val']+" } "
				+fields['result']+" {/if}";
			...
			if(isDebug){
				console.log(logTag+'sorted tpl sentence:['+exprStr+'] in Chrome-likes.');
			}
		}
		return exprStr;
	};
    ...
    var _callRender = function(){ //- wait longer?
        ...
        if(isDebug){
            console.log(logTag + "parse time cost: "
				+(((new Date()).getTime() - timeCostBgn)/1000) + "s");
        }
    };
    ...
    if(false){
        if(window.document.body){
            ...
            if(isDebug){
				console.log(logTag + " fire with document.onload "+(new Date()));
			}
        }
        else{
            _callRender();
 ...
 * 22:34 Sunday, August 4, 2019, + detailed comments
